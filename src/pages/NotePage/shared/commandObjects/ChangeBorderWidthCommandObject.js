...
export default class ChangeBorderWidthCommandObject extends CommandObject {
  ...
  execute(newWidth) {
    if (selectedObj !== null) {
      ...
      console.log('execute border width: ' + this.oldValue + " to " + this.newValue);
   ...
   */
  undo() {
    console.log('undo border width: from ' + this.newValue + " to " + this.oldValue);
    this.targetObject.borderWidth = this.oldValue;
    // maybe also need to fix the palette to show this object's color?
    this.undoHandler.changeCurrBorderWidthState(this.targetObject.selectedShapeId ,this.oldValue);
  }
   ...
   */
  redo() {
    console.log('redo border width from ' + this.oldValue + " to " + this.newValue);
    this.targetObject.borderWidth = this.newValue;
    // maybe also need to fix the palette to show this object's color?
    // becomeSelected(this.targetObject);
    this.undoHandler.changeCurrBorderWidthState(this.targetObject.selectedShapeId ,this.newValue);
  }
